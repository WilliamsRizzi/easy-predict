...
def _payment_requirements(resource_url: str) -> dict:
    return {
        'scheme':            'exact',
...
def _payment_required(resource_url: str):
    """Return HTTP 402 with the x402 payment challenge in body + headers."""
    body = {
        'x402Version': 1,
        'error':       'Payment Required: x402 micropayment required — send a signed payment in X-PAYMENT',
        'accepts':     [_payment_requirements(resource_url)],
...
def well_known_x402():
    """Machine-readable x402 resource list. Free, no payment."""
    resource = f'{PUBLIC_BASE_URL}/timeseries'
    return jsonify({
        'x402Version': 1,
        ...
        'resources': [{
            'resource': resource,
            'method':   'POST',
            'accepts':  [_payment_requirements(resource)],
        }],
    })
